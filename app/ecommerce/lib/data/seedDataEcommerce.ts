...
import { product, products_desserts } from "./placeDataEcom";
...
const seed = async (
  items: any[],
  DATABASE_NAME: string = "products_desserts",
) => {
  for (const item of items) {
    await (prisma as any)[DATABASE_NAME].create({
      data: item,
    });
  }
};
...
async function main() {
  try {
    ...
    // await seedRevenes();
    // await seedCustomers();
    await seed(products_desserts);
    await seed(product, "product");
